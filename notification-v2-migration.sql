-- Run this instead of the earlier notifications migration. It uses a clean table
-- so legacy notification columns cannot block likes or comments.
create table if not exists public.activity_notifications (
  ...
  kind text not null check (kind in ('post_like','post_comment','comment_reply','comment_like','mention','group_join')),
...
create policy "recipients read activity notifications" on public.activity_notifications for select using (recipient_id = auth.uid());
drop policy if exists "recipients update activity notifications" on public.activity_notifications;
create policy "recipients update activity notifications" on public.activity_notifications for update using (recipient_id = auth.uid()) with check (recipient_id = auth.uid());
